        ...
        <div class="row mb-3 justify-content-center">
            <div class="col-lg-7">
                <div class="add-form bg-white p-4 rounded">
                    ...
                    @endif
                    <form action="{{url('admin/update-our-work/'.$initiative->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="title" class="fs-5 mb-2">Title</label>
                            <input type="text" name="title" value="{{$initiative->title}}" id="title" class="w-100" placeholder="Enter work title..." required>
                        </div>

                        <div class="mb-3">
                            <label for="work_slug" class="fs-5 mb-2">Slug</label>
                            <input type="text" name="work_slug" value="{{$initiative->work_slug}}" id="work_slug" class="w-100" placeholder="Enter work title as slug..." required>
                        </div>

                        <div class="mb-3">
                            <label for="description" class="fs-5 mb-2">Description</label><br>
                            <textarea id="description" name="description" class="w-100" rows="5" placeholder="Enter Description...">{{$initiative->description}}</textarea>
                        </div>

                        <div class="mb-3">
                            <label for="meta_description" class="fs-5 mb-2">Meta Description</label><br>
                            <textarea id="meta_description" name="meta_description" class="w-100" rows="5">{{$initiative->meta_description}}</textarea>
                        </div>

                        <div class="mb-5">
                            <label for="image" class="fs-5 mb-2">Image</label><br>
                            <input id="image" name="image" class="w-100" type="file"><br><br>
                                <img src="{{asset('uploads/initiatives/'.$initiative->image)}}" width="100px" alt="existing image">
                        </div>
                        <button type="submit" class="btn btn-primary mb-1">Update Work</button>
